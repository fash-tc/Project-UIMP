...
export default function IncidentWizard({ alert, onClose }: Props) {
  ...
  const stepLabels = ['Jira Ticket', 'Statuspage'];
    ...
    <div className="bg-surface border border-orange/20 rounded-lg p-4 space-y-4">
      ...
      {step === 1 && (
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-muted mb-1">Jira Ticket Title</label>
            <input
              type="text"
              value={title}
              ...
              placeholder="Brief title for the internal Jira incident ticket"
            />
          </div>

          <div>
            <label className="block text-xs text-muted mb-1">
              Description <span className="text-muted/60">(internal — include hosts, IPs, and relevant technical detail)</span>
            </label>
            <textarea
              value={description}
              ...
              placeholder="Describe the incident with full internal detail — affected hosts, error messages, timeline, and any known cause."
            />
          </div>
          ...
          <div className="flex justify-end">
            <button
              onClick={() => { setStep(2); handleLoadComponents(); }}
              disabled={!title.trim()}
      ...
      {step === 2 && (
        <div className="space-y-3">
          ...
          {showStatuspage === true && !statuspageResult && (
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-muted mb-1">Statuspage Title <span className="text-orange">(customer-facing)</span></label>
                <input
                  type="text"
                  value={spTitle}
              ...
              <div>
                <label className="block text-xs text-muted mb-1">Statuspage Body <span className="text-orange">(customer-facing — avoid internal detail)</span></label>
                <textarea
                  value={spBody}
