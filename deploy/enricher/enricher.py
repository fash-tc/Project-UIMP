...
def _normalize_stale_alert_family_name(name):
    value = (name or "").strip().lower()
    if not value:
        return ""
...
def build_stable_zabbix_signature(alert):
    ...
    if "replicaset mismatch" in name_lc:
        ...
        "rabbitmq" in name_lc
        or "kubernetes" in name_lc
        or "pod [" in name_lc
        or "node [" in name_lc
        or tags.get("pod")
        or tags.get("node")
        or target in ("kubernetes", "nodes")
    ):
        alert_family = _normalize_stale_alert_family_name(name_lc)
    else:
        return None

    if "rabbitmq" in name_lc:
        target_family = "rabbitmq"
    elif target == "kubernetes" or tags.get("pod") or tags.get("replicaset") or "pod [" in name_lc:
        target_family = "kubernetes"
    elif target == "nodes" or tags.get("node") or "node [" in name_lc:
        target_family = "nodes"
    else:
